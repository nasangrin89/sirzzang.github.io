...
## 3. 해석
 ...
 그렇다면 FFN으로 오토인코더를 구성했을 때의 latent feature를 어떻게 해석해야 할지 궁금해 진다. FFN은 *그 나름대로* **자신의 네트워크에 맞추어** 차원을 축소하고 이해한다고 보아야 할 듯하다. 본래 이미지라는 것이 사람이 인식하는 형태가 있다 보니, FFN보다는 CNN이 *사람이 이해하기에* 좋은 형태로 이미지를 학습하고, 차원을 축소하는 데에 강점이 있다고 결론을 내리고 싶다.