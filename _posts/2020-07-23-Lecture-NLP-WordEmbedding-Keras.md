...
>  
>
>  기존에 우리가 다루었던 방식과 달리 `X_input_2`와 같이 모델을 구성해도 된다. 모델 내부에서 알아서 recurent step을 적용한다. 그러나 이 경우 배치 사이즈가 일정하게 고정되지 않아 학습에 시간이 오래 걸린다. **특히** 단어 임베딩의 경우는 일정한 사이즈로 배치 데이터를 구성해주어야 하기 때문에, 패딩을 진행해주는 것이 적합하다.
